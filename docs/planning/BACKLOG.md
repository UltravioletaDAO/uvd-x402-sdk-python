...
| 2026-09-13 | La autorizacion EIP-3009 usada vuelve OPACA del facilitador | `400 {"error": "contract_call_failed (ref: <uuid>)"}` (`x402-rs` `src/handlers.rs`, brazo `ContractCall`, que oculta la razon del revert a proposito). Ni codigo ni texto nombran el nonce, asi que `is_spent_nonce_error()` no lo ve en EVM contra este facilitador. Un token estable (`authorization_already_used`) sin la razon cruda lo cerraria | P1 — **NO ES DE ESTE REPO** | **ABIERTA 2026-09-13.** Medido en x402-rs 331d31d4: el brazo `ContractCall` usa `contract_call_failed` para `PayloadRejected` y `Unclassified` |
...
| 2026-09-14 | Las integraciones no pasan `idempotency_scope` | `decorators.py` y las integraciones de Flask, Django y Lambda llaman `process_payment()` sin scope: con `send_idempotency_key=True` en su config no mandan llave y loguean el aviso, porque no tienen de donde sacar el id de la compra. Opciones: un callback que lo lea del request, o documentar que ahi la llave no aplica | P2 | **ABIERTA 2026-09-14.** `grep -rn "idempotency_scope" src/uvd_x402_sdk/integrations src/uvd_x402_sdk/decorators.py` da vacio |
...
| 2026-09-14 | `verify_only()` no acepta `idempotency_scope` | Con la llave prendida loguea el aviso y no manda llave. `/verify` ignora el header hoy, asi que en el cable no cambia nada | P3 | **ABIERTA 2026-09-14.** `grep -n "def verify_only" -A 5 src/uvd_x402_sdk/client.py` no tiene el parametro |
