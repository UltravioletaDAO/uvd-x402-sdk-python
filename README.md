...
`derive_idempotency_key(payload, "settle", scope=...)` is `x402-settle-<sha256>` over the JSON
array `["x402-idempotency-scope/1", <signed block>, <scope>]` serialised with sorted keys and no
whitespace, non-ASCII as UTF-8 (`x402-verify-…` for verify). A signed block is always a JSON
object, so no block derives the key of a scoped call. The same authorization for the same purchase gives the same key in
...
- **Choosing the scope:** the scope is generated by the seller and stored with the purchase,
  never a value taken from the request. One value per purchase (an order id), created before the
  settle and stored with the purchase, so a retry after a restart reuses it. Never a value two purchases
