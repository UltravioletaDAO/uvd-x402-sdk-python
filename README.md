...
the details or the facilitator's JSON body) or `"wording"` for free text, matched by whole word.
...
- **Off switch: `X402Config.send_idempotency_key=False`.** Production runs the DynamoDB store, and a keyed settle the facilitator cannot check is `503 idempotency_store_unavailable` with no settle (fail-closed on purpose)
...
- **Better than the copy it came from.** tarotof matched substrings, and `"refused"` contains `"used"`: the facilitator's own transient `NonceOrMempool` message ("the node refused this transaction on nonce or mempool grounds and never queued it") read as a spent authorization. Matching is by whole word now, pinned with that message. And `idempotency_key_conflict` is in the code set, because the facilitator caches only successes: under the derived key, a conflict means this authorization settled
...
- 1086 tests pass (1052 before, 34 added, none lost)
