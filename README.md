...
| Arc | EVM | 5042 | `eip155:5042` | Active |
| Arc Testnet | EVM | 5042002 | `eip155:5042002` | Active |
...
> **Arc uses one USDC balance at two precisions.** Native gas uses 18 decimals; ERC-20 payments use 6. Both Arc networks sign `$0.01` as `10000` atomic units. Do not add the native and ERC-20 balance readings together.
...
### Idempotency-Key: one purchase binding per payment (on by default since 0.89.0)

```python
from uvd_x402_sdk import new_idempotency_key

# Nothing to do for the common case: every process_payment() call carries one
# fresh random key on its /verify, its /settle and the settle's timeout fallback.
result = client.process_payment(x_payment_header, Decimal("0.10"))
result.idempotent_replayed   # True: the facilitator's replay of THIS purchase's settle

# To resume a payment after a restart, create the key first and store it with the order.
order.idempotency_key = new_idempotency_key()   # "x402-<64 hex>", merchant-private
save(order)
result = client.process_payment(x_payment_header, Decimal("0.10"),
                                idempotency_key=order.idempotency_key)
```

`verify_payment()`, `settle_payment()`, `try_settle_payment()` and `process_payment()` send an
`Idempotency-Key` on every call. One handling of a payment (a `process_payment()` call, or a
`settle_payment()` call with its retries and timeout fallback) carries **one** key on every request
it makes. The key is random unless the call brings its own: `idempotency_key` (a key the seller
created with `new_idempotency_key()` and stored with the order) or `idempotency_scope` (a key
derived from the payment and a secret order id, see below). `VerifyResponse`, `SettleResponse` and
`PaymentResult` report the key in `idempotency_key`; it stays out of `model_dump()`, so it never
reaches the buyer in `PAYMENT-RESPONSE`.

- **Why random.** A facilitator with durable receipts (x402-rs 2.39.0, Arc and native Hedera today)
  hands an admitted payment's answer back only to the binding that admitted it: the same
  `Idempotency-Key` or the same `X-UVD-Purchase`. Possession of the signed payment alone is not a
  binding. A key derived from the `X-PAYMENT` is recomputed by whoever holds the payment, so a buyer
  resending their own `X-PAYMENT` in a new request would make the seller send the same key and get
  the original settle back. A random key per handling cannot be recomputed.
- **Why the same key on verify and settle.** The rail binds only the key that admitted the payment.
  0.83.0 to 0.88.0 sent `x402-verify-…` on `/verify`, so the `/verify` of a retried purchase read
  `authorization_already_settled` for a payment that had settled.
- **A replay nobody on this request admitted is refused.** A handling with no binding of its own (a
...
- **`idempotency_scope`** is kept: `derive_idempotency_key(payload, "settle", scope=...)` is
  ...
  0.83.1 to 0.88.0 sent, now on `/verify` too. **It is a binding only while the scope is a secret
  of the seller** (an order id the seller generates, which the buyer does not know and cannot
  guess). A scope taken from the request, or a sequential one, gives the key to whoever holds the
  payment. `idempotency_key` and `idempotency_scope` at once raise `ValueError`; a malformed key
  (not 1-255 visible ASCII characters, or starting with `receipt:`) raises before anything is sent.
- **Off switch: `send_idempotency_key=False`** sends no key at all, as 0.83.1 to 0.88.0 did by
  default. Then a settle that timed out after being admitted gets `409 authorization_already_settled`
  on its fallback: probably the seller's own payment, but nothing proves it. The SDK raises the
  facilitator's `FacilitatorError` with the receipt; reconcile `receipt.settlement` with your own
  records before delivering anything.

What the facilitator does with the key:

| Request | Without receipts (x402-rs 2.28.0 `post_settle`) | With receipts (x402-rs 2.39.0) |
|---|---|---|
| same key, same request, payment settled | cached `200`, `Idempotent-Replayed: true` | original `200`, `Idempotent-Replayed: true` |
| same key, payment in flight | executes again (no lock on the key) | `202 settlement_in_progress`: transient, same request later |
| no key or another key, payment settled | executes again; EVM answers an opaque `400 contract_call_failed` | `409 authorization_already_settled`; `/verify`: `isValid: false` |
| no key or another key, payment in flight | executes again | `409 authorization_in_flight`; `/verify`: `isValid: false` |
| same authorization, other terms or another purchase context | `409 idempotency_key_conflict` (same key) | `409 receipt_request_conflict` |
| the facilitator cannot read its store | `503 idempotency_store_unavailable`, **no settle** | `503 receipt_store_unavailable`, no verdict |
| `/verify` | header ignored | stored verdict for the binding |

- **With the key on, the idempotency store is a dependency on networks without receipts.** A keyed
...
`admitted_authorization_code(exc)` names the receipt rail's three refusals, and
`payment_conflict_response(exc)` builds the answer. The SDK's middlewares and decorators (FastAPI,
Flask, Django, Lambda, `require_payment`) use it, never deliver on these refusals, and never answer
them with 402:
...
from uvd_x402_sdk import (is_spent_nonce_error, is_transient_error,
                          payment_conflict_response, transient_503_response)
...
> **An index, not a ledger.** Records are written best-effort *after* settlement,
> so an outage loses rows while payments proceed — verify anything that matters
> against the transaction hash. Counting starts when the operator enabled the
> store, so earlier operations are **unknown, not zero**. And unless failure
> publishing is on, operations that error are not recorded at all, so a 100%
> success rate means "no failures were recorded".
>
> `get_transactions` has **no pagination**: it returns the newest N (capped at
...
### v0.84.0 (2026-09-16)
- Add Arc mainnet (`arc`, `eip155:5042`) and testnet (`arc-testnet`, `eip155:5042002`) to network lookup, defaults, signing and v1/v2 envelopes.
- Use USDC `0x3600000000000000000000000000000000000000`, 6 payment decimals and EIP-712 domain `USDC` / `2`; retain 18 decimals for native gas metadata.
- Validate payment scaling, domains and isolation between the two networks. See [Arc usage](docs/networks/arc.md) for examples and verification evidence.
- 27 enabled networks, including 17 EVM networks (14 mainnets and 3 testnets). Arc supports direct `exact` EOA payments; other Arc payment modes are not advertised.
