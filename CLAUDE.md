...
- `is_spent_nonce_error()` / `spent_nonce_evidence()` ported from tarotof (`api/main.py`): codes first (normalised, nested), wording second **by whole word** — tarotof's substring match read "refused" as "used", so the facilitator's transient `NonceOrMempool` message was a false positive. `idempotency_key_conflict` is in the code set on purpose. On EVM the UVD facilitator answers a used authorization with an opaque `400 contract_call_failed (ref)`: the classifier cannot see it there
