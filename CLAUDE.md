...
### Idempotency-Key & spent nonce (client.py, v0.83.0)
- `verify_payment()` / `_settle_once()` / `_check_settle_fallback()` send `Idempotency-Key: derive_idempotency_key(payload, op)` = `x402-<verify|settle>-<sha256>` over the signed `payload` block (sorted keys, no whitespace). Namespaced by operation; the requirements do NOT enter it. Off with `X402Config.send_idempotency_key=False`
...
- Pinned against a real-socket stub of that contract in `tests/test_idempotency_local_facilitator.py`, not the binary. It does not catch a NEW authorization for the same purchase, and `process_payment()` verifies first (verify has no cache)
