...
- **LA TRAMPA.** Arc paga el gas en USDC, asi que **el mismo saldo** se publica dos veces: `eth_getBalance` a **18** decimales (gas) y `balanceOf` a **6** (pagos), con `balanceOf(a) == eth_getBalance(a) // 10**12`. Medido en vivo sobre 12 direcciones de un bloque real: exacto en todas (`tests/fixtures/arc-testnet-balances.json`). **El importe de un pago x402 viaja en 6.** Un 18 en el registro multiplica cada cobro por 10^12: `$0.01` se firma como `10000000000000000` en vez de `10000`. El 18 esta escrito en `extra_config["native_gas_decimals"]` como hecho de la cadena y **no lo lee nadie** que calcule un importe; `tests/test_arc_testnet.py` monta ese estado malo a proposito y se pone rojo (3 de sus 17 tests fallan con 18)
...
### Idempotency-Key & spent nonce (client.py, v0.83.0; opt-in since v0.83.1)
- **Opt-in since 0.83.1.** `X402Config.send_idempotency_key` defaults to `False`, and with it on a key goes out only on calls that pass `idempotency_scope` (the caller's own purchase id, generated by the seller and stored with the purchase, never a value taken from the request) to `verify_payment` / `settle_payment` / `try_settle_payment` / `process_payment`. On without a scope (`None` or blank) = no header + one `logger.warning` per process (`_warn_missing_idempotency_scope`); a non-str scope raises `TypeError` before anything is sent. Why: a key derived from the payment alone cannot tell two purchases of the same price apart, because their settle bodies are byte-identical
- `_facilitator_headers()` (used by `verify_payment()` and `_settle_once()`, whose headers `_check_settle_fallback()` reuses) sends `Idempotency-Key: derive_idempotency_key(payload, op, scope)` = `x402-<verify|settle>-<sha256>`: without scope over the signed `payload` block (sorted keys, no whitespace; the 0.83.0 vector, unchanged), with scope over the array `["x402-idempotency-scope/1", block, scope]` (after 0.83.1; 0.83.1 hashed `{"payload", "scope"}`). The block is always a JSON object, so no block derives a scoped key: domain separation pinned by a test with a block shaped like the 0.83.1 material. Pinned vectors: unscoped, scoped ASCII, scoped non-ASCII (`ensure_ascii=False`, UTF-8 bytes). Namespaced by operation; the requirements do NOT enter it; the block stays in, so two sellers with the same order id do not collide
